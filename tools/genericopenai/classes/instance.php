...
/**
 * Instance class for the connector instance of aitool_genericopenai.
 *
...
class instance extends base_instance {
    ...
    protected function extend_form_definition(\MoodleQuickForm $mform): void {
        ...
        // Remove the existing model select element and replace it with a text input
        $mform->removeElement('model');
        $textelementparams = ['style' => 'width: 100%'];
        $mform->addElement('text', 'model', get_string('model', 'local_ai_manager'), $textelementparams);
    ...
    protected function extend_store_formdata(stdClass $data): void {
        ...
        // Use the user-provided endpoint if available, otherwise fall back to the default OpenAI API endpoint
        $endpoint = !empty($data->endpoint) ? trim($data->endpoint) : 'https://api.openai.com/v1/chat/completions';
